...
// Older recovery links deliver their tokens in the URL fragment
// (#access_token=...&refresh_token=...). Fragments are never sent to the
// server, so those links can only be resolved here in the browser.
async function resolveSessionFromHash(): Promise<boolean> {
  const hash = window.location.hash.startsWith('#') ? window.location.hash.slice(1) : ''
  const params = new URLSearchParams(hash)

  if (params.get('error_code') ?? params.get('error')) return false
  ...
  // Drop the tokens from the address bar once they're in the session.
  window.history.replaceState(null, '', window.location.pathname)
  return true
}

export default function ResetPasswordClient({ initialReady }: { initialReady: boolean }) {
  ...
  // The server already told us whether a recovery token or session is present,
  // so nothing to resolve in the common case. This effect only covers links
  // that deliver their tokens in the URL fragment (#access_token=...), which
  // the server never sees because fragments aren't sent in the request.
  useEffect(() => {
    if (initialReady) return

    let cancelled = false
    resolveSessionFromHash().then((resolved) => {
      if (cancelled) return
