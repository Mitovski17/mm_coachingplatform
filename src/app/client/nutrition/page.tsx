...
export default function ClientNutritionPage() {
  ...
  const [planType, setPlanType] = useState<'training' | 'rest'>('rest')
  ...
  // Load client + auto-detect plan type from today's workout
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      ...
      try {
        const t = await getTodayTemplate(c.id)
        if (cancelled) return
        if (!planTypeUserSet) setPlanType(t ? 'training' : 'rest')
      } catch {
        // ignore
      }
    })()
  ...
  // Load meal plan when planType or clientId changes
  useEffect(() => {
    if (!clientId) return
    let cancelled = false
    ;(async () => {
      const plan = await getActiveMealPlan(clientId, planType)
      if (cancelled) return
      setMealPlan(plan)
    })()
    return () => {
  ...
  // Load day logs whenever date or client changes
  ...
  // Goal: sum of Option A foods across all meals
  const goal = useMemo(() => {
...
function WeekStrip({
  ...
  const days = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      arr.push({ iso, label: labels[i], num: d.getDate(), isToday: iso === today })
    }
    return arr
  }, [selectedDate, today])
